...
describe("MLB market insights route", () => {
  it("returns fixture envelope with games", async () => {
    const mod = await import("@/app/api/widgets/mlb-market-insights/route");
    const res = await mod.GET(new Request("http://localhost/api/widgets/mlb-market-insights?mode=beginner&dataMode=fixture"));
  ...
  it("moneyline isFallback when THE_ODDS_KEY absent", async () => {
    const mod = await import("@/app/api/widgets/mlb-market-insights/route");
    const res = await mod.GET(new Request("http://localhost/api/widgets/mlb-market-insights?mode=advanced&dataMode=fixture"));
    const body = await res.json();
  ...
  it("userFacingMessage mentions MLB games when games present", async () => {
    const mod = await import("@/app/api/widgets/mlb-market-insights/route");
    const res = await mod.GET(new Request("http://localhost/api/widgets/mlb-market-insights?mode=beginner&dataMode=fixture"));
    const body = await res.json();

    expect(body.data.userFacingMessage).toMatch(/MLB game/);
  });
});
