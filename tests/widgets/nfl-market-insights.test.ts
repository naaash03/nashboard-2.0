...
describe("NFL market insights route", () => {
  it("returns fixture envelope with games", async () => {
    const mod = await import("@/app/api/widgets/nfl-market-insights/route");
    const res = await mod.GET(new Request("http://localhost/api/widgets/nfl-market-insights?mode=beginner&dataMode=fixture"));
  ...
  it("moneyline isFallback when THE_ODDS_KEY absent", async () => {
    const mod = await import("@/app/api/widgets/nfl-market-insights/route");
    const res = await mod.GET(new Request("http://localhost/api/widgets/nfl-market-insights?mode=advanced&dataMode=fixture"));
    const body = await res.json();
  ...
  it("userFacingMessage mentions NFL games when games present", async () => {
    const mod = await import("@/app/api/widgets/nfl-market-insights/route");
    const res = await mod.GET(new Request("http://localhost/api/widgets/nfl-market-insights?mode=beginner&dataMode=fixture"));
    const body = await res.json();

    expect(body.data.userFacingMessage).toMatch(/NFL game/);
  });

  it("games include DAL @ PHI from fixture", async () => {
    const mod = await import("@/app/api/widgets/nfl-market-insights/route");
    const res = await mod.GET(new Request("http://localhost/api/widgets/nfl-market-insights?mode=beginner&dataMode=fixture"));
    const body = await res.json();

    const teams = body.data.games.flatMap((g: { awayTeam: string; homeTeam: string }) => [g.awayTeam, g.homeTeam]);
    expect(teams.some((t: string) => t.includes("Cowboys"))).toBe(true);
    expect(teams.some((t: string) => t.includes("Eagles"))).toBe(true);
  });
});
