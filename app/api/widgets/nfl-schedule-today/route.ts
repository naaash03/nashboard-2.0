...
export async function GET(req: Request) {
  ...
  try {
    ...
    const contract = toWidgetPayload({
      ...
      notes: [
        "NFL schedule from ESPN scoreboard.",
    ...
    return NextResponse.json({
      data: {
        ...
        userFacingMessage: games.length > 0
          ? `Showing ${games.length} NFL game${games.length > 1 ? "s" : ""} today.`
    ...
    const message = `Failed to load NFL schedule: ${String(error)}`;
    ...
    return NextResponse.json(
      {
        data: {
          ...
          dateUsed: todayISO,
          games: [],
          userFacingMessage: "Could not load today's NFL schedule.",
        },
        meta,
        contract,
        error: message,
