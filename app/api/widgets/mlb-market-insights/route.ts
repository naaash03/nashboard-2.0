...
import { getTodaysSchedule } from "@/lib/providers/espn/mlb";
  ...
  try {
    ...
    const enriched: EnrichedGame[] = await Promise.all(
      games.map(async (game): Promise<EnrichedGame> => {
        // Fetch current odds first; then pass current ML to line movement so
        // the shift can be computed without a second current-odds call.
        const odds = await fetchGameOdds("baseball_mlb", game.homeTeam.name, game.awayTeam.name);
    ...
    const contract = toWidgetPayload({
      ...
      notes: [
        "Games from ESPN MLB scoreboard.",
    ...
    return NextResponse.json({
      data: {
        ...
        userFacingMessage: games.length > 0
          ? `Showing market data for ${games.length} MLB game${games.length > 1 ? "s" : ""} today.`
    ...
    const message = `Failed to load MLB market insights: ${String(error)}`;
