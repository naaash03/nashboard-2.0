...
export async function GET(req: Request) {
  ...
  try {
    ...
    const contract = toWidgetPayload({
      ...
      notes: [
        "Games from ESPN NBA scoreboard.",
    ...
    return NextResponse.json({
      data: {
        ...
        userFacingMessage: games.length > 0
          ? `Showing market data for ${games.length} NBA game${games.length > 1 ? "s" : ""} today.`
    ...
    const message = `Failed to load NBA market insights: ${String(error)}`;
