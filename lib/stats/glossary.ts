...
export const BUILTIN_GLOSSARY_TERMS: GlossaryTerm[] = [
  ...
  term({
    ...
    key: "opp_ppg",
    label: "Opp PPG",
    aliases: ["opponent points per game", "points allowed per game", "defensive points per game"],
    plainDefinition: "Opponent points per game is the average points a team allows each game.",
    whyItMatters: "It is the quickest read on team scoring defense.",
    direction: "LOWER_IS_BETTER",
  }),
  term({
    sport: "NBA",
    key: "net_rating",
    label: "Net Rating",
    aliases: ["net rtg", "net rating", "point differential per game"],
    plainDefinition: "Net rating here is points scored per game minus points allowed per game.",
    whyItMatters: "It captures both offense and defense in one number — positive means a team outscores opponents on average.",
    direction: "HIGHER_IS_BETTER",
    interpretation: "A positive net rating means the team outscores opponents on average; the larger the gap, the more dominant.",
  }),
  term({
    sport: "NBA",
    key: "fg_pct",
    label: "FG%",
    aliases: ["field goal percentage", "field goal pct"],
    plainDefinition: "Field goal percentage is made field goals divided by attempted field goals.",
    whyItMatters: "It is a basic read on shooting accuracy, though it does not weight threes or free throws.",
    direction: "HIGHER_IS_BETTER",
  }),
  term({
    sport: "NBA",
    key: "streak",
    label: "Streak",
    aliases: ["win streak", "losing streak", "current streak"],
    plainDefinition: "Streak is the number of consecutive wins (W) or losses (L) a team currently has.",
    whyItMatters: "It signals momentum heading into the next game beyond the raw record.",
    direction: "CONTEXT_DEPENDENT",
  }),
  term({
    sport: "NFL",
