...
export default function NbaMarketInsightsWidget(props: WidgetCommonProps) {
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/widgets/nba-market-insights?${params}`);
  ...
  return (
    <div className="flex flex-col gap-2 p-3 h-full text-white">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[9px] text-zinc-600 uppercase tracking-wide font-medium">NBA</span>
      </div>
      ...
      {!loading && !error && data && data.games.length === 0 && (
        <div className="text-[11px] text-zinc-500 mt-1">No NBA games scheduled today.</div>
      )}
        ...
        {meta && (
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={() => props.onReportBug({ widget: "nba_market_insights", meta })}
            >
              Report issue
            </button>
          </div>
