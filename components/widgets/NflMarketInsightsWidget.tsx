...
export default function NflMarketInsightsWidget(props: WidgetCommonProps) {
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/widgets/nfl-market-insights?${params}`);
  ...
  return (
    <div className="flex flex-col gap-2 p-3 h-full text-white">
      ...
      {!loading && !error && data && data.games.length === 0 && (
        <div className="text-[11px] text-zinc-500 mt-1">No NFL games scheduled today.</div>
      )}
        ...
        {meta && (
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={() => props.onReportBug({ widget: "nfl_market_insights", meta })}
            >
              Report issue
            </button>
          </div>
